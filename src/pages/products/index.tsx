...
const ProductsPage: React.FC = () => {
  ...
  const handleImage = async (p: Product) => {
    try {
      ...
      await uploadProductImage(p.id, base64); Taro.showToast({ title: '图片已保存', icon: 'success' }); load();
        ...
        list.map(p => (
          ...
          >
            <View className={styles.listItem}>
              {p.image_url && <Image className={styles.thumb} src={/^https?:/.test(p.image_url) ? p.image_url : `${getBaseUrl()}${p.image_url}`} mode="aspectFill" />}
              ...
              <View className={styles.itemActions}>
                <View className={styles.btnIn} onClick={e=>{e.stopPropagation();handleImage(p);}}>图片</View>
