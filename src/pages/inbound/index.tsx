...
import { getProducts, stockIn, getTransactions, getCustomers, deleteTransaction, syncUpload } from '@/services/api';
...
const InboundPage: React.FC = () => {
  ...
  const handleConfirm = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await stockIn(selectedId, qty, operator, remark, customerId);
      await syncUpload({
        action: 'stock_in',
        type: 'in',
      ...
      }).catch(error => console.error('[Inbound] sync upload failed', error));
    ...
    } catch (e) { console.error('[Inbound] confirm failed', e); Taro.showToast({ title: String((e as any)?.message || '入库失败'), icon: 'none' }); }
  ...
  const handleDeleteRecent = async (tx: Transaction) => {
    ...
    try {
      ...
      await syncUpload({ action: 'transaction_delete', type: tx.type, id: tx.id, product_id: tx.product_id, quantity: tx.quantity, customer_id: tx.customer_id, remark: tx.remark, operator: tx.operator }).catch(error => console.error('[Inbound] delete sync failed', error));
      Taro.showToast({ title: '已删除', icon: 'success' });
      loadProducts();
      loadRecent();
    }
    catch (e) { console.error('[Inbound] delete failed', e); Taro.showToast({ title: String((e as any)?.message || '删除失败'), icon: 'none' }); }
  };
        ...
        {selectedProduct && (
          <View className={styles.productInfo}>
            ...
            <View className={styles.infoTextWrap}>
              <Text className={styles.infoName}>商品名称：{selectedProduct.name}</Text>
